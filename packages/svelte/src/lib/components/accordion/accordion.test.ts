...
describe('Accordion', () => {
  ...
  it('should render accordion items', async () => {
    ...
    expect(screen.getByText('What is Codesign UI?')).toBeInTheDocument()
  ...
  it('should show default expanded item', () => {
    ...
    // Codesign UI is open by default
    const codesignContent = screen.getByText('A headless component library for building accessible web apps.')
    expect(codesignContent).toBeVisible()
