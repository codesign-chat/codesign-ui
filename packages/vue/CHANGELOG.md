...
- Fix `./hotkeys` and `./interaction` entrypoints missing from the published `exports` map. The build files shipped, but
  ...
  `interaction` primitives were added, so `import { useHotkeys } from '@codesign-ui/react/hotkeys'` failed with
...
```tsx
import { createContext } from '@codesign-ui/solid/utils'
```
...
- **General**: Fix issue where some Zag packages were not included in the package.json `dependencies` and
  `devDependencies`. This causes import errors when using the `@codesign-ui/vue` package.
...
- **Collection**: Fixed issue where entrypoint `@codesign-ui/vue/collection` was not working as expected
...
```tsx
import { PasswordInput } from '@codesign-ui/vue/password-input'
...
- **[NEW] Listbox**: Introduced the `Listbox` component for selecting a single or multiple items from a list. See the
  [documentation](https://codesign.chat/docs/components/listbox) for details.
...
- **[NEW] AngleSlider**: Introduced the `AngleSlider` component for selecting an angle. See the
  [documentation](https://codesign.chat/docs/components/angle-slider) for details.
- **[NEW] FloatingPanel**: Introduced the `FloatingPanel` component for creating floating windows. See the
  [documentation](https://codesign.chat/docs/components/floating-panel) for details.
...
- **Field**: Fixed `Textarea` to use `codesign.textarea`, ensuring support for the `asChild` prop.
...
```tsx
import { DownloadTrigger } from '@codesign-ui/react/download-trigger'
...
- **[Breaking] Splitter**: Redesigned splitter machine to support more use cases and improve DX. Check out the
  [Splitter](https://codesign.chat/docs/components/splitter) documentation for more details.
...
Codesign UI just got a major performance boost! 🚀
...
In our stress tests with **10,000 components**, Codesign v5 delivered **1.5x–4x** better performance across the board.
