...
```tsx
import { createContext } from '@codesign-ui/react/utils'
```
...
- **Menu**: Fix `Menu.ItemText` not working inside `Menu.TriggerItem`
  ...
  ```tsx
  import { Select } from '@codesign-ui/react/select'
...
- **Collection**: Fixed issue where entrypoint `@codesign-ui/react/collection` was not working as expected
...
```tsx
import { PasswordInput } from '@codesign-ui/react/password-input'
...
![Performance comparison showing Codesign v5 is 1.5x-4x faster than other libraries](./v5.svg)
