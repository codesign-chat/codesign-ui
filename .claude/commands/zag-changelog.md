---
description: Sync changelog updates from chakra-ui/zag repo to all Codesign UI framework changelogs
---
...
2. Determine which Zag versions are new
   ...
   - Compare the latest Codesign UI release changelog (`packages/react/CHANGELOG.md`) against the Zag changelog
   - Identify Zag versions whose changes are NOT yet reflected in the Codesign UI changelogs
...
4. Review the changes with me before applying
   ...
   - Confirm which changes should be synced to Codesign UI
...
5. Create a changeset file in `.changeset/`
   ...
   - Include all four frameworks: `@codesign-ui/react`, `@codesign-ui/solid`, `@codesign-ui/svelte`, `@codesign-ui/vue`
...
- Only sync changes that are relevant to Codesign UI components
