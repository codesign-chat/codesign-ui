# Codesign UI - Framework-Specific Patterns
...
#### Native Elements: Use `codesign` Factory

In Solid components, **always use `codesign.<element>` instead of native HTML elements** (e.g., `<div>`, `<span>`,
`<button>`):

```tsx
import { codesign } from '../factory'

// ✅ CORRECT - Use codesign factory
<codesign.div {...props} />
<codesign.span data-kind="label">{text}</codesign.span>
<codesign.button onClick={handler}>Click</codesign.button>
...
- The `codesign` factory uses `Dynamic` internally, which defers element creation to render time and is SSR-safe
...
<template>
  <codesign.div v-bind="component.getRootProps()" :ref="component.refs.rootRef">
    <slot />
  </codesign.div>
</template>
```
...
### Codesign Component Factory

```ts
// All frameworks use the codesign factory for polymorphic components
<codesign.div {...props} asChild={asChild}>  // Vue
<Codesign as="div" {...props} {asChild}>     // Svelte
<codesign.div {...props} asChild={asChild}>  // React/Solid
```
...
;<Menu.Item value="docs" asChild>
  <a href="https://codesign-ui.com">Documentation</a>
</Menu.Item>
```
...
;<Menu.Item
  value="docs"
  asChild={(itemProps) => (
    <a href="https://codesign-ui.com" {...itemProps()}>
      Documentation
    </a>
  )}
/>
...
```vue
<script setup lang="ts">
import { Menu } from '@codesign-ui/vue/menu'
</script>
...
```svelte
<script lang="ts">
  import { Menu } from '@codesign-ui/svelte/menu'
</script>

<Menu.Item value="docs">
  {#snippet asChild(itemProps)}
    <a href="https://codesign-ui.com" {...itemProps()}>Documentation</a>
  {/snippet}
</Menu.Item>
```
...
<template>
  <codesign.div v-bind="component.getRootProps()" :as-child="asChild">
    <slot />
  </codesign.div>
</template>
```
...
<!-- Codesign component with bind:ref and @attach -->
<Codesign as="div" bind:ref {...mergedProps} {@attach setNode} />
```
...
- `<Codesign as="div" />` - Polymorphic component factory (void elements only)
...
```tsx
// Direct imports
import { FieldRoot, FieldLabel } from '@codesign-ui/react/field'
