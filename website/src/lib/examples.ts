...
const { CODESIGN_PLUS_API_KEY, CODESIGN_PLUS_URL } = process.env

export const fetchExamples = async (): Promise<Example[]> => {
  const examples = await fetch(`${CODESIGN_PLUS_URL}/api/examples`, {
    cache: 'no-cache',
    headers: {
      Authorization: CODESIGN_PLUS_API_KEY,
    },
  })
    .then((res) => res.json())
    .catch(() => [])
...
export const fetchExample = async (id: string): Promise<Example> => {
  const example: Example = await fetch(`${CODESIGN_PLUS_URL}/api/examples/${id}`, {
    headers: {
      Authorization: CODESIGN_PLUS_API_KEY,
    },
    cache: 'no-cache',
  ...
  return {
    ...example,
    previewUrl: [CODESIGN_PLUS_URL, example.previewUrl].join(''),
  }
}
...
export const fetchCodeExamples = async (props: FetchCodeExamplesParams): Promise<CodeExample[]> => {
  ...
  const sources: SourceFile[] = await fetch(`${CODESIGN_PLUS_URL}/api/examples/${id}/sources/${framework}`, {
    headers: {
      Authorization: CODESIGN_PLUS_API_KEY,
    },
    cache: 'no-cache',
