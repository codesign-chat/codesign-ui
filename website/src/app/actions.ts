...
export const hasUserPermission = async () => {
  ...
  return session?.purchases.some((purchase) => purchase.startsWith('codesign-ui-plus'))
}
    ...
    try: () =>
      fetch(`https://${process.env.FRESHDESK_DOMAIN}.freshdesk.com/api/v2/tickets`, {
        ...
        body: JSON.stringify({
          ...
          subject: `[Codesign UI] ${formData.get('subject')}`,
          description: formData.get('message'),
          status: 2,
          priority: 1,
          source: 2,
          tags: ['codesign-ui', 'website-contact'],
        }),
      }),
