...
To handle this, Codesign UI includes the `EnvironmentProvider`, allowing you to set the appropriate root node or
document, ensuring correct DOM queries.

## Setup

To support custom environments like an iframe, Shadow DOM or Electron, render the `EnvironmentProvider` component to
provide the environment context to all Codesign UI components.
...
import { EnvironmentProvider } from '@codesign-ui/react'
