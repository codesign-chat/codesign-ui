...
In Codesign UI, the `asChild` prop lets you integrate custom components, ensuring consistent styling and behavior while
promoting flexibility and reusability. All Codesign components that render a DOM element accept the `asChild` prop.
...
## The Codesign Factory

You can use the `codesign` factory to create your own elements that work just like Codesign UI components.
...
Any props you pass to `codesign.span` are merged onto the child element, which is how the factory forwards styling and behavior to
whatever you render.
