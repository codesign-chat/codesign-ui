...
Codesign UI is a headless component library that works with any styling solution. It provides functional styles for elements
...
If you prefer using classes instead of data attributes, utilize `class` or `className` prop to add custom classes to Codesign
UI components.
...
Codesign UI does not prescribe a global z-index scale. You decide the base z-index value for overlays, and Codesign exposes Zag's
...
> **Important:** When importing anatomy objects, we recommend using the `@codesign-ui/<framework>/anatomy` entrypoint (e.g.,
> `@codesign-ui/react/anatomy`) instead of the main package export to avoid potential build and import errors.
