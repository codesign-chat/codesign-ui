...
```tsx
import { Combobox } from '@codesign-ui/<framework>/combobox'
...
```tsx
const Combobox: CodesignCombobox.RootComponent = (props) => {
  return <CodesignCombobox.Root {...props}>{/* ... */}</CodesignCombobox.Root>
}
```
