...
```tsx
import { TreeView as CodesignTreeView } from '@codesign-ui/react/tree-view'

const TreeView: CodesignTreeView.RootComponent = (props) => {
  return <CodesignTreeView.Root {...props}>{/* ... */}</CodesignTreeView.Root>
}
```
