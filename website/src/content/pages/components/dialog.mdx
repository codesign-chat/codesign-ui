...
freeing up resources. Prefer this over conditionally rendering `Dialog.Root`—see **Conditional Rendering**.
...
Unmounting `Dialog.Root` when toggling open state can break focus, scroll lock, and cleanup. Keep the root mounted and
...
const Do = () => {
  return (
    <Dialog.Root open={isOpen} onOpenChange={(e) => setOpen(e.open)} lazyMount unmountOnExit>
      <Portal>...</Portal>
    </Dialog.Root>
  )
}
...
Use a shared overlay z-index with `--layer-index` to keep stacked dialogs and nested overlays in order:
...
Read the [Styling guide](/docs/guides/styling#z-index) for more details.
