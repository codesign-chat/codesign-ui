...
```tsx
const Select: CodesignSelect.RootComponent = (props) => {
  return <CodesignSelect.Root {...props}>{/* ... */}</CodesignSelect.Root>
}
```
...
```tsx
import { Select, useSelectContext } from '@codesign-ui/react/select'
