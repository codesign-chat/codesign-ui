...
Storybook is a great way to develop and document your design system in isolation. Codesign UI works well with it — you just
need to know how the pieces fit together. Here's a walkthrough of setting up Storybook with Codesign UI React.
...
Codesign UI uses compound components — `Checkbox.Root`, `Checkbox.Control`, `Checkbox.Label`, etc. Each story composes these
...
By default, Storybook won't pick up Codesign UI's component props. This is because the props flow through `@zag-js` types in
...
Codesign UI's compound component pattern maps naturally to Storybook stories. Write your stories the same way you'd use the
