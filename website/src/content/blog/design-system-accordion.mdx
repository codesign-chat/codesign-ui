...
description: How to build an accordion component with Codesign UI and Panda CSS
date: '2025-06-15'
author: 'Esther'
tags: ['accordion', 'codesign-ui', 'design-system']
---
...
With Codesign UI, you get unstyled but accessible building blocks. This means you have full control and can style the
...
Before we dive into specific styles, it's essential to understand the parts that make up the Codesign UI Accordion
component. Each part supports the `data-scope` and `data-part` attributes, which you can target in both Vanilla CSS and
Panda CSS.
...
We’ll start by defining foundational styles for each part of the `Accordion` component using Codesign UI’s attributes
like `data-scope="accordion"` and `data-part="..."`.
...
Although Codesign UI doesn’t include an `item-body` part by default, we’ll create this part ourselves when applying
styles. The item body serves as a wrapper around the content inside each accordion item. Let's style it:
...
```tsx
import { Accordion } from '@codesign-ui/react'
import { ChevronDownIcon } from 'lucide-react'
...
action, [**check out Storybook**](https://github.com/estheragbaje/codesign-react-vanilla-css/blob/main/stories/menu/menu.stories.tsx).
...
We also use `accordionAnatomy.keys()` from Codesign UI to automatically pull in all the part names for the Accordion
component.
...
import { accordionAnatomy } from '@codesign-ui/react/accordion'
...
Although not provided by default in Codesign UI, we’ll add a custom `itemBody` part to manage spacing within item
content. To style this custom part, we’ll need to extend our accordion recipe.
...
by Codesign UI, we can easily add our own by introducing a custom slot named **`itemBody`**.
...
Now apply the generated class names to each corresponding Codesign UI component part using the `className` prop.
...
the [**Storybook examples**](https://github.com/estheragbaje/codesign-react-panda-css/tree/main/src/stories/menu).

## Conclusion

You’ve just built a fully custom-styled Accordion component using Codesign UI, with support for Vanilla CSS or Panda
CSS.

We walked through the base styles, size and visual variants, and added a custom part (`item-body`) to give you more
layout control.

A headless library like Codesign UI means that whichever style pattern you decide to use, you're in control of your
design system and can scale as your team grows. and keep things clean.

Feel free to explore the [GitHub repo](https://github.com/estheragbaje/codesign-react-panda-css) or
[Storybook examples](https://github.com/estheragbaje/codesign-react-panda-css/tree/main/src/stories/accordion).
