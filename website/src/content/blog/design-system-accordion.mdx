...
Before we dive into specific styles, it's essential to understand the parts that make up the Codesign UI Accordion component.
...
We’ll start by defining foundational styles for each part of the `Accordion` component using Codesign UI’s attributes like
...
Although Codesign UI doesn’t include an `item-body` part by default, we’ll create this part ourselves when applying styles.
...
Although not provided by default in Codesign UI, we’ll add a custom `itemBody` part to manage spacing within item content. To
...
You’ve just built a fully custom-styled Accordion component using Codesign UI, with support for Vanilla CSS or Panda CSS.

We walked through the base styles, size and visual variants, and added a custom part (`item-body`) to give you more
layout control.

A headless library like Codesign UI means that whichever style pattern you decide to use, you're in control of your design
