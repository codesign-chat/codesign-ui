...
description: How to build a menu component with Codesign UI and Panda CSS
date: '2025-06-13'
author: 'Esther'
tags: ['menu', 'codesign-ui', 'design-system']
---

The [Menu component](https://codesign.chat/docs/components/menu) is used in web apps for dropdowns, actions, or
contextual options.
...
- **Grouped Sections:** We'll style grouped menu items, including section labels and visual separators, to help organize
  long lists.
- **Checkbox & Radio Items:** Codesign UI supports checkboxes and radio buttons inside menus. We'll organize long lists
  of menu items with styled section labels and visual separators.
...
Codesign UI exposes helpful attributes like `data-scope="menu"` and `data-part="..."` for every part of the menu, which
we'll use to style each part.
...
keyframes earlier so apply these animations using Codesign UI's built-in `data-state="open"` or `data-state="closed"`
attribute:
...
- **Highlighted state:** When a user hovers over a menu item or navigates to it using the keyboard, Codesign UI
  automatically applies the `data-highlighted` attribute. We can use this to indicate the active item
...
- **Disabled state:** In many menus, not all option are available. For example, a "Delete" option may be unavailable for
  users without permission. Codesign UI provides the `data-disabled` attribute to style the disabled styles
  ```css
  [data-scope='menu'][data-part='item'] {
  	// item styles here
    }
...
Codesign UI automatically applies `data-state="open"` when the menu is expanded. We added a style such that when the
menu opens, the indicator smoothly rotates to point right
...
When an item is selected, Codesign UI applies `data-state="checked"` attribute for styling.
...
- `data-state="checked"`: When an item is selected, Codesign UI applies this attribute, allowing us to visually
  distinguish it (e.g., by making the font bold).
...
For organizing long lists of menu items, Codesign UI offers grouping capabilities with labels and visual separators:
...
Codesign UI automatically applies `data-part="item-text"`, `"data-part='item-indicator"` and
`data-part="item-command"`to the relevant parts.
...
With these base styles and size variants defined in your `menu.css` , applying them to your Codesign UI Menu component
is a matter of adding the correct `className` to the `Menu.Trigger`, `Menu.Content` and other parts.
...
[check out Storybook](https://github.com/estheragbaje/codesign-react-vanilla-css/blob/main/stories/menu/menu.stories.tsx).
...
We also use `menuAnatomy.keys()` to automatically retrieve all the part names defined in Codesign UI's anatomy for the
menu component.
...
import { menuAnatomy } from '@codesign-ui/react/menu'
...
When building interactive menus, you often need to include selectable items like checkboxes and radio buttons. Codesign
UI provides dedicated components like `Menu.CheckboxItem` and `Menu.RadioItem` for this purpose.
...
- The `&[data-type]` selector targets all menu items that have a `data-type` attribute. Codesign UI assigns
...
For organizing longer lists of menu items into logical sections, Codesign UI provides support for menu groups. This
includes dedicated parts for both group labels and visual separators.
...
Codesign UI provides the `itemText` and `itemIndicator` parts, but for a custom "command" text, we'll extend our recipe.

**Extending the Menu Anatomy for Custom Parts**

Panda CSS's recipe system is quite flexible. While `menuAnatomy.keys()` gives us all the standard parts exposed by
Codesign UI, we can easily add our own by introducing a custom slot named `itemCommand`.

This explicitly tells Panda CSS to expect and generate styles for this new part, even though it's not part of Codesign
UI's default structure.
...
With your menu recipe now fully defined, applying these styles to your Codesign UI components is a seamless process.
...
**3. Apply Classes to Codesign UI Parts**

Finally, apply the generated class names to the corresponding Codesign UI component parts using the `className` prop.
...
[Storybook examples](https://github.com/estheragbaje/codesign-react-panda-css/tree/main/src/stories/menu).

## **Conclusion**

Styling Codesign UI's Menu component is flexible whether you're working with Vanilla CSS or Panda CSS.
