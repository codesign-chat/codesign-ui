...
The [Menu component](https://codesign.chat/docs/components/menu) is used in web apps for dropdowns, actions, or contextual
options.
...
- **Grouped Sections:** We'll style grouped menu items, including section labels and visual separators, to help organize
  long lists.
- **Checkbox & Radio Items:** Codesign UI supports checkboxes and radio buttons inside menus. We'll organize long lists of
...
Codesign UI exposes helpful attributes like `data-scope="menu"` and `data-part="..."` for every part of the menu, which we'll
...
- **Highlighted state:** When a user hovers over a menu item or navigates to it using the keyboard, Codesign UI automatically
...
Codesign UI automatically applies `data-state="open"` when the menu is expanded. We added a style such that when the menu
...
- `data-state="checked"`: When an item is selected, Codesign UI applies this attribute, allowing us to visually distinguish
...
Codesign UI automatically applies `data-part="item-text"`, `"data-part='item-indicator"` and `data-part="item-command"`to the
relevant parts.
...
With these base styles and size variants defined in your `menu.css` , applying them to your Codesign UI Menu component is a
...
We also use `menuAnatomy.keys()` to automatically retrieve all the part names defined in Codesign UI's anatomy for the menu
component.
...
When building interactive menus, you often need to include selectable items like checkboxes and radio buttons. Codesign UI
...
For organizing longer lists of menu items into logical sections, Codesign UI provides support for menu groups. This includes
...
Panda CSS's recipe system is quite flexible. While `menuAnatomy.keys()` gives us all the standard parts exposed by Codesign
UI, we can easily add our own by introducing a custom slot named `itemCommand`.

This explicitly tells Panda CSS to expect and generate styles for this new part, even though it's not part of Codesign UI's
default structure.
